...
- Strict Codex Sol admission now forces fresh usage through the auth broker and its per-credential cache, so a cached Plus report cannot admit a downgraded Free account.
- Strict Codex Sol selection reuses its fresh entitlement result after unchanged OAuth resolution, while credential revision or authority changes still require another fresh check.
