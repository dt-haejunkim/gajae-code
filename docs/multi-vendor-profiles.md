...
- **openai-codex on a ChatGPT account** has entitlement-dependent model access: base GPT includes `gpt-5.5` and `gpt-5.4`. Standalone `-codex` variants (`gpt-5.3-codex`, `gpt-5.2-codex`, `gpt-5.1-codex-max` / `-mini`) return `not supported when using Codex with a ChatGPT account`. GPT-5.6 Sol access for Plus and unknown plans is decided by live provider entitlement: those local usage labels only rank Sol accounts, while confirmed Free candidates never dispatch Sol and an all-Free pool fails locally; provider-confirmed entitlement failures surface actionable guidance. Spark models retain their existing local filter: if any connected account is confirmed Pro, non-Pro candidates are excluded; if none is confirmed Pro, Plus or unknown-plan candidates may still be attempted.
